# 00. 用語・境界定義
    ...
    ## プロダクト境界
    `ASTER` は **Persona Contract Compiler** を提供する。
    会話UI、アカウント管理、課金、本人確認、汎用モデレーションUIは外部システムの責務とする。

    ## 主要エンティティ
    - `Persona`: Personaに関する不変IDを持つドメイン要素。
- `PersonaVersion`: PersonaVersionに関する不変IDを持つドメイン要素。
- `Trait`: Traitに関する不変IDを持つドメイン要素。
- `VoiceRule`: VoiceRuleに関する不変IDを持つドメイン要素。
- `PromptFragment`: PromptFragmentに関する不変IDを持つドメイン要素。
- `ToolGrant`: ToolGrantに関する不変IDを持つドメイン要素。
- `PolicyReference`: PolicyReferenceに関する不変IDを持つドメイン要素。
- `CompiledBundle`: CompiledBundleに関する不変IDを持つドメイン要素。
